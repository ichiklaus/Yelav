...
import { PortfolioProps } from '@interfaces/index';
import { useTranslations } from 'next-intl';

export default function Portfolio() {
  const t = useTranslations();
  const selected_engineer_work = t.raw('selected_engineer_work') as PortfolioProps[];

  const projects = selected_engineer_work.sort(function (a, b) {
  ...
  return (
    <section id='projects' className='pt-32 -mt-32'>
      <AnimatePresence>
        <header>
          <h2 className='font-bold text-accent mb-6'>{t('selected_engineer_work_section.title')}</h2>
        </header>
        <section key={'portfolio-section-01'} className='flex flex-col gap-y-8'>
